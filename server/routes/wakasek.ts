...
wakasek.use('*', authMiddleware, requireRole(['WAKASEK', 'ADMIN']));

wakasek.get('/aspirations', async (c) => {
  try {
...
wakasek.get('/aspirations/export', async (c) => {
  try {
...
wakasek.get('/aspirations/:id', async (c) => {
  try {
...
wakasek.patch('/aspirations/:id', async (c) => {
  try {
    const id = parseInt(c.req.param('id'), 10);
    if (isNaN(id)) {
      return c.json({ success: false, error: 'ID tidak valid.' }, 400);
...
wakasek.post('/aspirations/bulk-delete', async (c) => {
  const user = c.get('user');
  if (user.role !== 'ADMIN') {
